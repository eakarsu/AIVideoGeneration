...
// Video editing suggestions recommending cuts/transitions
// Feature: video-editing-suggestions
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AIVideoGeneration" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: Video editing suggestions recommending cuts/transitions.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'video-editing-suggestions', payload, result);
    res.json({ success: true, feature: 'video-editing-suggestions', ...result });
  } catch (err) {
    console.error('video-editing-suggestions analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AIVideoGeneration". Return strict JSON only.',
      `Feature: Video editing suggestions recommending cuts/transitions. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'video-editing-suggestions', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'video-editing-suggestions', title: 'Video editing suggestions recommending cuts/transitions', project: 'AIVideoGeneration' });
});

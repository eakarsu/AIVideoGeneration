...
export default function CfCollaborationLayerWithTimecodeAnchoredReviewComments() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-collaboration-layer-with-timecode-anchored-review-comments/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-collaboration-layer-with-timecode-anchored-review-comments', project: 'AIVideoGeneration' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-collaboration-layer-with-timecode-anchored-review-comments/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Collaboration layer with timecode-anchored review comments</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AIVideoGeneration</p>
      </div>

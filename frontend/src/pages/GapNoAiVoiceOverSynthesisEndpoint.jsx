...
export default function GapNoAiVoiceOverSynthesisEndpoint() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-ai-voice-over-synthesis-endpoint/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-ai-voice-over-synthesis-endpoint', project: 'AIVideoGeneration' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-ai-voice-over-synthesis-endpoint/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No AI voice-over synthesis endpoint</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_ai · AIVideoGeneration</p>
      </div>

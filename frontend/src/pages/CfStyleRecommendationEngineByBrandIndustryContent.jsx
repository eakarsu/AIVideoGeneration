...
export default function CfStyleRecommendationEngineByBrandIndustryContent() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-style-recommendation-engine-by-brand-industry-content-type/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-style-recommendation-engine-by-brand-industry-content-type', project: 'AIVideoGeneration' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-style-recommendation-engine-by-brand-industry-content-type/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Style recommendation engine by brand/industry/content type</h1>

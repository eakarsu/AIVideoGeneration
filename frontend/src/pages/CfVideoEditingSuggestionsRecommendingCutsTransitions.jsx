...
export default function CfVideoEditingSuggestionsRecommendingCutsTransitions() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-video-editing-suggestions-recommending-cuts-transitions/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-video-editing-suggestions-recommending-cuts-transitions', project: 'AIVideoGeneration' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-video-editing-suggestions-recommending-cuts-transitions/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Video editing suggestions recommending cuts/transitions</h1>
